...
def convert_annotation(year, image_id, list_file):
    ...
    for obj in root.iter('object'):
        ...
        if (b[3] - b[1]) * (b[2] - b[0]) <= 0:
            continue
        list_file.write(" " + ",".join([str(a) for a in b]) + ',' + str(cls_id))
        nums[classes.index(cls)] = nums[classes.index(cls)] + 1

    size_obj = root.find('size')
    width, height = size_obj.find('width').text, size_obj.find('height').text
    list_file.write(" " + str(int(height)) + "," + str(int(width)))
...
def main():
    ...
    if annotation_mode == 0 or annotation_mode == 1:
        ...
        num = len(total_xml)
        list = random.sample(range(num), int(photo_per * num))
        num = len(list)
    ...
    if annotation_mode == 0 or annotation_mode == 2:
        ...
        for year, image_set in VOCdevkit_sets:
            image_ids = open(os.path.join(save_path, '{}.txt'.format(image_set)),
                             encoding='utf-8').read().strip().split()
            list_file = open(os.path.join(save_path, '%s.txt' % image_set), 'w', encoding='utf-8')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--am', type=int, default=0, help='annotation_mode')
    ...
    annotation_mode = args.am
    #   必须要修改，用于生成2007_train.txt、2007_val.txt的目标信息
    #   与训练和预测所用的classes_path一致即可
    #   如果生成的2007_train.txt里面没有目标信息
    #   那么就是因为classes没有设定正确
    #   仅在annotation_mode为0和2的时候有效
    classes_path = args.cp
    #   trainval_percent用于指定(训练集+验证集)与测试集的比例，默认情况下 (训练集+验证集):测试集 = 1:0
    #   train_percent用于指定(训练集+验证集)中训练集与验证集的比例，默认情况下 训练集:验证集 = 9:1
    #   photo_per是(训练集+验证集)占图片的比例
    #   仅在annotation_mode为0和1的时候有效
    trainval_percent = args.tvt
    train_percent = args.t
    photo_per = args.pp
    #   指向VOC数据集所在的文件夹
    #   默认指向根目录下的VOC数据集
    VOCdevkit_path = args.voc_p
